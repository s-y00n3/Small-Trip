					...
					<div id="main">
						...
						<!-- Posts -->
							...
							<article style="width:100%;">
							 ...
							 String totalSql = "select count(*) from review"; 
					 ...
					 int pagePerRow = 10;
				    String listSql = "SELECT * FROM review ORDER BY id DESC LIMIT ?, ?";
...
<%
	if(totalRowCount==0){
		%>
		<tr>
			<td colspan="3">작성된 여행 일기가 없습니다.</td>
...
%>
                  <tr>
                   <td> <a href="reviewView.jsp?id=<%=listResultSet.getInt("id")%>"><%=listResultSet.getString("title")%></a></td>
                    <td><a href="reviewView.jsp?id=<%=listResultSet.getInt("id")%>"><%=listResultSet.getString("start_date")%></a></td>
                    <td><a href="reviewView.jsp?id=<%=listResultSet.getInt("id")%>"><%=listResultSet.getString("finish_date")%></a></td>
                </tr>
<%        
            }
	}
%>
    ...
    <div>
       <button onclick="location.href='write_diary.jsp'">여행 계획 작성하기</button>
    </div>
